<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Resumo </title>
<link rel="stylesheet" type="text/css" href="css/resumo.css">
<link rel="stylesheet" type="text/css" href="css/bootstrap.min.css">
</head>
<body>
	<section>
		<div class="container-fluid">
			<div class="container print">
				<div class=" txt-sd text-center mb-1">
					<h3 class="txt-saudacao">Gerencie os jogadores</h3>
				</div>
				<%//implemntar quando for fazer o back %>
				<div class="textoo">
					<div>	
						<h4 class="font">Placar:</h4>
						<div>� espera de um select </div>
					</div>	
					<div>	
						<h4 class="font">Maiores pontuadores:</h4>
						<div>� espera de um select (nome e somatoria de pontos)</div>
					</div>
					<div>
						<h4 class="font">Supervisores da partida:</h4>
						<div>� espera de um select (nome deles)</div>
					</div>
					...
					
				</div>
				
		  </div>
		</div>
	</section>

</body>
</html>